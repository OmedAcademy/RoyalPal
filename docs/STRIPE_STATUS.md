...
| #   | Blocker                                                               | Who      |
| --- | --------------------------------------------------------------------- | -------- |
| 1   | Apply `docs/PENDING_MIGRATIONS.sql` (migration 0026) to the remote DB | operator |
...
Migrations `0001`–`0025` **are** applied remotely and verified. `0026` is
**not**. It is written idempotently, so it is safe to re-run if a paste into
the SQL editor fails partway.
